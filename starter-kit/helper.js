...
async function testChapter1() {
    ...
    try {
        ...
        if (
            responsePayload.greeting?.includes("connected to DSQL successfully")
        ) {
...
async function runStressTest(config) {
    ...
    console.log(
        `Parallel requests per batch: ${parallelCalls.toLocaleString()}`,
    );
    ...
    for (let iteration = 0; iteration < iterations; iteration++) {
        ...
        process.stdout.write(
            `\r[${progress.padStart(3, " ")}%] ${completed
                ...
                5,
                " ",
            )} calls/s | Success: ${successRate.padStart(
                5,
                " ",
            )}% | ${elapsed}s`,
        );
    ...
    if (Object.keys(stats.errorTypes).length > 0) {
        ...
        console.log(`Each worker handles ${parallelCallsPerWorker} parallel calls per iteration, for ${iterations} iterations`);
            ...
            const workerPromise = new Promise((resolve, reject) => {
                ...
                worker.on("message", (msg) => {
                    if (msg.type === "progress") {
                        ...
                        const totalCompleted = workerCompletedCounts.reduce((a, b) => a + b, 0);
                        ...
                        process.stdout.write(
                            `\r[${progress.padStart(3, " ")}%] ${totalCompleted
                                .toString()
                                .padStart(7, " ")}/${TOTAL_CALLS.toLocaleString()} calls | ${rate.padStart(
                                5,
                                " ",
                            )} calls/s | ${elapsed}s`,
                        );
                ...
                worker.on("exit", (code) => {
                    if (code !== 0) {
                        ...
                        reject(new Error(`Worker ${workerId + 1} stopped with exit code ${code}`));
                    }
                });
            });
        ...
        for (const stats of allStats) {
            ...
            aggregatedStats.minDuration = Math.min(aggregatedStats.minDuration, stats.minDuration);
            aggregatedStats.maxDuration = Math.max(aggregatedStats.maxDuration, stats.maxDuration);
            aggregatedStats.totalRetries += stats.totalRetries;
            aggregatedStats.maxRetries = Math.max(aggregatedStats.maxRetries, stats.maxRetries);
        ...
        if (aggregatedStats.success > 0) {
            const avgDuration = aggregatedStats.totalDuration / aggregatedStats.success;
            console.log("Lambda Execution Times:");
            console.log(`  Min:                ${aggregatedStats.minDuration.toFixed(2)}ms`);
            console.log(`  Max:                ${aggregatedStats.maxDuration.toFixed(2)}ms`);
            console.log(`  Avg:                ${avgDuration.toFixed(2)}ms`);
            console.log();
        }
        ...
        if (Object.keys(aggregatedStats.errorTypes).length > 0) {
            console.log("Error Breakdown:");
            for (const [errorType, count] of Object.entries(aggregatedStats.errorTypes).sort(
                (a, b) => b[1] - a[1],
            )) {
                console.log(`  ${errorType}: ${count.toLocaleString()}`);
            }
        ...
        if (typeof responsePayload.balance === "number") {
            ...
            try {
                ...
                allTransactions.forEach((row, idx) => {
                    console.log(
                        `  ${idx + 1}. ID: ${row.id}, Payer: ${
                            row.payer_id
                        }, Payee: ${row.payee_id}, Amount: ${
                            row.amount
                        }, Time: ${row.created_at}`,
                    );
                });
...
async function setupChapter6() {
    ...
    try {
        ...
        if (currentCount < TARGET_ACCOUNTS) {
            ...
            for (let workerId = 0; workerId < numWorkers; workerId++) {
                const workerStartId =
                    currentCount + workerId * accountsPerWorker + 1;
                ...
                const workerPromise = new Promise((resolve, reject) => {
                    const worker = new Worker(
                        path.join(__dirname, "insert-worker.js"),
                        {
                            workerData: {
                                workerId: workerId + 1,
                    ...
                    worker.on("message", (msg) => {
                        if (msg.type === "progress") {
                            console.log(
                                `  [Worker ${msg.workerId}] Inserted accounts ${msg.startId.toLocaleString()} to ${msg.endId.toLocaleString()}`,
                            );
                        } else if (msg.type === "done") {
                            resolve();
                        } else if (msg.type === "error") {
    ...
    for (let i = start; i <= end; i++) {
        ...
        try {
            ...
            if (responsePayload.errorType || responsePayload.errorMessage) {
                isError = true;
                if (
                    responsePayload.errorMessage?.includes(
                ...
                const match =
                    responsePayload.transaction_time.match(/^([\d.]+)ms$/);
                if (match) {
